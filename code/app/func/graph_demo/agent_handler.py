...
class AgentHandler:
    ...
    async def stream_chat(
        ...
        Yields:
            dict: 与 Dify SSE 兼容的事件，包含 event, task_id, message_id, answer 等字段
        """
    ...
    async def blocking_chat(
        ...
        for msg in reversed(messages):
            if hasattr(msg, "content") and (not hasattr(msg, "tool_calls") or result.get("tool_calls", []) == []):
                answer = msg.content
                break
        return answer
